...
$parentModule = 'CmdFav'
if (-not $ModuleName) { $ModuleName = 'CmdFav.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\CmdFav'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\CmdFav.psd1"
$prereqHash = @{
